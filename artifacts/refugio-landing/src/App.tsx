...
function HeroArtwork() {
  return (
    <div className="relative min-h-[320px] overflow-hidden rounded-[1.5rem] border border-[#06392f]/20 bg-[#06392f] lg:min-h-[590px] lg:rounded-[2rem]" aria-label="Ilustração abstrata de uma porta aberta para um espaço de escuta" role="img">
      <div className="orb absolute -right-16 -top-16 h-56 w-56 rounded-full border border-[#06392f]/20 bg-[#06392f]/65 md:h-72 md:w-72" />
      <div className="orb-delay absolute -bottom-20 -left-20 h-64 w-64 rounded-full border border-[#06392f]/15 bg-[#06392f]/80 md:h-80 md:w-80" />
      <svg viewBox="0 0 560 620" className="absolute inset-0 h-full w-full" preserveAspectRatio="xMidYMid slice" aria-hidden="true">
        ...
        <path d="M280 302c-19 43-27 81-20 122 6 35 14 61 20 78 6-17 14-43 20-78 7-41-1-79-20-122Z" fill="#06392f" opacity=".8" />
        <g fill="#06392f" opacity=".55">
          <circle cx="88" cy="122" r="3" /><circle cx="466" cy="188" r="3" /><circle cx="438" cy="98" r="2" /><circle cx="125" cy="212" r="2" />
        </g>
        <g stroke="#06392f" strokeWidth="1" opacity=".32">
...
function IntentOption({ intent, title, copy, selected, onSelect }: { intent: Intent; title: string; copy: string; selected: boolean; onSelect: (intent: Intent) => void }) {
  return (
    <button type="button" className={`intent-card ${selected ? 'is-selected' : ''}`} aria-pressed={selected} onClick={() => onSelect(intent)} data-testid={`button-intent-${intent}`}>
      ...
      <span className="mt-8 block text-[.78rem] leading-[1.45] text-[#06392f]/70">{copy}</span>
    </button>
  );
}
    ...
    <form onSubmit={submit} noValidate className="rounded-[1.5rem] border border-[#06392f]/20 bg-[#f3ebd6]/80 p-5 md:p-7" data-testid="form-waitlist">
      <div className="mb-7">
        <p className="eyebrow">entre na lista de interesse</p>
         ...
         <p className="mt-3 max-w-md text-sm leading-relaxed text-[#06392f]/65">A gente te avisa quando abrir — e vai querer saber o que você espera encontrar aqui.</p>
      </div>
      <div className="space-y-2">
        <label htmlFor="refugio-email" className="text-xs font-bold">Seu melhor e-mail</label>
        <div className="relative">
          <Mail size={16} className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-[#06392f]/50" />
        ...
        <p id="signup-hint" className="text-[.7rem] text-[#06392f]/55">Só vamos usar para avisar sobre os próximos passos.</p>
      </div>
      ...
      {error && <p id="signup-error" role="alert" className="mt-4 text-xs font-semibold text-[#06392f]" data-testid="status-signup-error">{error}</p>}
      <button type="submit" disabled={submitting} className="button-primary mt-7 w-full disabled:cursor-wait disabled:opacity-70" data-testid="button-submit-signup">{submitting ? 'Salvando seu convite...' : 'Quero entrar na lista de espera'} {!submitting && <ArrowRight size={16} />}</button>
       <p className="mt-4 flex items-center justify-center gap-1.5 text-center text-[.68rem] leading-relaxed text-[#06392f]/55"><LockKeyhole size={12} /> Grátis e anônimo. Você entra se quiser — e a gente te avisa quando o Refúgio abrir.</p>
    </form>
  );
}
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
   ...
   return (
     <div className="compact-waitlist mt-8 max-w-2xl">
      ...
      <p className="mt-3 text-xs text-[#06392f]/55">Grátis. Anônimo. Sem compromisso.</p>
    </div>
  );
}
...
function FaqItem({ question, answer }: { question: string; answer: string }) {
  ...
  return (
    <div className="border-b border-[#06392f]/20">
      ...
      {open && <p className="max-w-3xl pb-6 text-sm leading-[1.7] text-[#06392f]/65">{answer}</p>}
    </div>
  );
}
    ...
    return (
      <main className="refugio-page grain flex min-h-screen items-center justify-center px-5 py-12">
        <form onSubmit={signIn} className="w-full max-w-md rounded-[1.5rem] border border-[#06392f]/20 bg-[#f3ebd6] p-7 md:p-9">
          ...
          <p className="mt-4 text-sm leading-relaxed text-[#06392f]/65">Digite a senha de administrador para consultar os dados da validação.</p>
    ...
    <main className="refugio-page grain">
      ...
      <section id="top" className="mx-auto grid w-full max-w-7xl gap-8 px-5 pb-14 pt-5 lg:grid-cols-[1.05fr_.95fr] lg:items-center lg:gap-16 md:px-10 md:pb-32 md:pt-16">
        <div className="max-w-2xl">
           <div className="reveal is-visible flex items-center gap-3" data-testid="text-hero-eyebrow"><span className="h-px w-9 bg-[#06392f]" /><span className="eyebrow">para quem precisa falar e não tem com quem</span></div>
           <h1 className="serif reveal reveal-delay-1 is-visible mt-6 max-w-5xl text-[clamp(3.1rem,7.5vw,7rem)] leading-[.89] text-[#06392f]" data-testid="text-hero-title">Tire o peso do peito hoje. <span className="text-[#06392f]">Fale sem revelar quem você é.</span></h1>
           <p className="reveal reveal-delay-2 mt-8 max-w-2xl text-[1.06rem] leading-[1.55] text-[#06392f]/72 md:mt-10 md:text-[1.2rem]">Uma comunidade anônima para desabafar e ser acolhido por pessoas que já passaram por algo parecido. <strong>Sem seguidores, sem exposição</strong>, sem precisar fingir que está tudo bem.</p>
          <div className="reveal reveal-delay-3 is-visible mt-9 flex flex-wrap items-center gap-6 md:mt-11">
            <a href="#entrar" className="button-primary" data-testid="button-hero-join">Quero entrar na lista de espera <ArrowDownRight size={16} /></a>
             <span className="max-w-[12rem] text-[.7rem] leading-relaxed text-[#06392f]/55">Grátis. Sem compromisso. Avisaremos quando abrir.</span>
          </div>
        </div>
        <HeroArtwork />
      </section>
          ...
          <div className="grid gap-12 md:grid-cols-[.72fr_1.28fr] md:gap-24">
            <div>
               ...
               <h2 className="serif mt-4 max-w-md text-5xl leading-[.95] md:text-6xl">E se você pudesse falar sem que ninguém soubesse quem você é?</h2>
                <p className="mt-6 max-w-sm text-sm leading-[1.65] text-[#06392f]/65">Aqui você não precisa usar seu nome, colocar foto ou montar um perfil. Sem a pressão de ser reconhecido, fica mais fácil dizer o que está sentindo.</p>
             </div>
             <div className="grid gap-0 border-t border-[#06392f]/20">
              {[
                 ...
                 <div key={number} className="grid grid-cols-[3rem_1fr] gap-4 border-b border-[#06392f]/20 py-7 md:grid-cols-[4.5rem_1fr] md:gap-7">
                   <div className="flex flex-col items-start gap-4"><span className="text-xs font-bold text-[#06392f]">{number}</span><Icon size={20} strokeWidth={1.6} /></div>
                   <div><h3 className="text-lg font-bold tracking-[-.025em]">{title}</h3><p className="mt-2 max-w-lg text-sm leading-[1.6] text-[#06392f]/62">{copy}</p></div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="mx-auto max-w-7xl">
          <div ref={principles.ref} className={principles.className}>
            <div className="flex flex-col justify-between gap-10 md:flex-row md:items-end">
               <div><p className="eyebrow">um lugar sem personagem</p><h2 className="serif mt-4 max-w-2xl text-5xl leading-[.94] md:text-7xl">Aqui, ninguém precisa<br /><span className="text-[#06392f]">parecer forte.</span></h2></div>
      ...
      <section id="entrar" className="mx-auto grid w-full max-w-7xl gap-12 px-5 py-24 md:grid-cols-[.9fr_1.1fr] md:items-center md:gap-24 md:px-10 md:py-36">
        <div ref={formSection.ref} className={formSection.className}>
          <p className="eyebrow">o próximo passo</p>
           <h2 className="serif mt-4 text-5xl leading-[.93] md:text-7xl">Talvez você não precise ser forte<br /><span className="text-[#06392f]">o tempo todo.</span></h2>
            <p className="mt-7 max-w-md text-[1.04rem] leading-[1.6] text-[#06392f]/68">Quando a comunidade abrir, você vai ter um lugar para falar sem revelar quem é. Do outro lado, pessoas estarão prontas para ouvir. Deixe seu e-mail e a gente te avisa.</p>
            <div className="mt-10 flex items-center gap-4 border-t border-[#06392f]/15 pt-5 text-xs text-[#06392f]/56"><span className="flex h-8 w-8 items-center justify-center rounded-full border border-[#06392f]/25"><Mail size={14} /></span> Grátis. Anônimo. Sem fins lucrativos.</div>
        </div>
        <SignupForm />
      </section>
      ...
      <footer className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-5 py-8 text-xs text-[#06392f]/55 md:flex-row md:items-center md:justify-between md:px-10">
        <Mark />
    ...
    <main className="refugio-page grain">
      ...
      <section id="top" className="mx-auto grid w-full max-w-7xl gap-8 px-5 pb-14 pt-5 lg:grid-cols-[1.05fr_.95fr] lg:items-center lg:gap-16 md:px-10 md:pb-32 md:pt-16">
        <div className="max-w-2xl">
          ...
          <h1 className="serif mt-5 text-[clamp(2.75rem,11vw,7rem)] leading-[.9] text-[#06392f]">Tem um peso que você carrega porque não tem com quem falar. <span className="text-[#06392f]">Aqui, você fala — e ninguém sabe quem é você.</span></h1>
          <p className="mt-8 max-w-2xl text-[1.06rem] leading-[1.55] text-[#06392f]/72 md:text-[1.2rem]">Um lugar anônimo para desabafar e ser ouvido por quem entende. Você fala do seu jeito, na hora que quiser — sem seguidores, sem exposição e sem precisar fingir que está tudo bem.</p>
      ...
      <section className="paper-crease px-5 py-16 md:px-10 md:py-32">
         ...
         <div><p className="eyebrow">a virada</p><h2 className="serif mt-4 max-w-2xl text-5xl leading-[.94] md:text-7xl">E se você pudesse falar sem que <span className="text-[#06392f]">ninguém</span> soubesse quem você é?</h2></div>
          <div><p className="text-[1.05rem] leading-[1.7] text-[#06392f]/70">Aqui você não precisa usar seu nome, colocar foto ou montar um perfil. Sem a pressão de ser reconhecido, fica mais fácil dizer o que está sentindo.</p><p className="serif mt-8 text-3xl leading-tight text-[#06392f] md:text-5xl">O anonimato não é para se esconder — é o que te devolve a liberdade de falar.</p><p className="mt-8 text-[1.05rem] leading-[1.7] text-[#06392f]/70">Do outro lado, alguém lê e responde — muitas vezes alguém que já passou por isso e sabe, na pele, como é precisar de apoio.</p></div>
        </div>
      </section>
      ...
      <section id="como-funciona" className="mx-auto max-w-7xl px-5 py-16 md:px-10 md:py-32">
        ...
        <div className="mt-12 grid gap-0 border-t border-[#06392f]/20 md:grid-cols-3">
          {[
          ...
          ].map(([number, title, copy]) => <div key={number} className="border-b border-[#06392f]/20 py-7 md:border-r md:px-7 md:first:pl-0 md:last:border-r-0"><span className="text-xs font-bold text-[#06392f]">{number}</span><h3 className="mt-8 text-xl font-bold">{title}</h3><p className="mt-3 text-sm leading-[1.6] text-[#06392f]/65">{copy}</p></div>)}
        </div>
      </section>
